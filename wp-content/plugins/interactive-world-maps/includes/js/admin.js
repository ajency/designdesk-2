...
function iwmdataToTable() {
    ...
    if(oldText == 'categories_count'){
        categories_count_label = '<strong>Categories Count</strong><br>The map will try to fetch the categories of your posts to populate the map automatically.<br> The preview below will not display anything, but once you apply the shortcode to a page, it will render the map with the categories names and links to the categories archive page.'
...
function isolink() {
    ...
    if (resolution == 'metros' && display == "regions") {
        ...
        span.innerHTML = '<b><i class="fa fa-question-circle"></i> ' + document.getElementById('region')[document.getElementById('region').selectedIndex].innerHTML + '</b> - To create your interactive regions, use these three-digit <a href="https://support.google.com/richmedia/answer/2745487?hl=en" target="_blank">metropolitan area codes</a> as the region codes.'; // 
        latlonhide();
    }
    ...
    if (display == 'customicon') {
        ...
        span.innerHTML = '<b><i class="fa fa-question-circle"></i> Custom Icon (Coordinates Code)</b> - When using the Custom Icon display mode, the image you have specified in the settings page will be added to the specified location.<br> You should insert the coordinates values in the Region Code, in this format: latitude longitude.<br> <strong>Do not use commas, use a space to separate de values</strong>. Example:34.3071438 -53.7890625 <br><br> <strong>Color Field</strong> will be ignored, but it\'s still mandatory'; // 
        latlonshow();
    ...
    if (mapaction == 'i_map_action_content_right_1_4' || mapaction == 'i_map_action_content_right_1_3') {
        ...
        span.innerHTML = span.innerHTML + '<br /><br /><b><i class="fa fa-question-circle"></i> Action - Display content on the right</b> - The content will display to the right of the map. You can use the "description" field to set the default value to display before any region or marker is clicked.'; // 
        actionvaluetip.innerHTML = 'Content to display on the right of the map'
        //customoptionshow();
...
function iwm_tinymce(placeid) {
    ...
    if (editor == '1') {
        ...
        tinymce.init({
            ...
            plugins: "image fullscreen lists media wordpress wpautoresize wpeditimage wplink wpdialogs wpview code",
            menubar: false,
...
function iwm_populate(id,datadir,current,thisbutton) {
    ...
    fetch(datadir+'/'+id+'.txt')
      ...
      .then(text => { current.val(text); iwmdataToTable(); } );
